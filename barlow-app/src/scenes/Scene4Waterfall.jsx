    ...
    <div>
      ...
      <section style={s.section}>
        ...
        <div style={s.formGrid}>
          <FormRow label="Interest Proceeds ($)">
            <input style={s.input} type="number" value={form.interest}
              onChange={e => setForm(f => ({ ...f, interest: e.target.value }))} />
          </FormRow>
          <FormRow label="Principal Proceeds ($)">
        ...
        <button style={{ ...s.btnPrimary, marginTop: 16 }} onClick={() => calculate()} disabled={isRunning}>
        ...
        <section style={s.section}>
          ...
          {hasDiversion ? (
            <>
              ...
              <table style={s.table}>
                <thead><tr>
                  <Th>Triggering Test</Th><Th>Diversion Amount</Th><Th>Cure Mechanism</Th>
                </tr></thead>
                <tbody>
                  {diversionLedger.entries.map((d, i) => (
                    <tr key={i}>
                      <Td><code>{d.triggering_test}</code></Td>
                      <Td style={{ color: 'var(--color-fail)', fontWeight: 600 }}>{fmtM(d.diversion_amount)}</Td>
                      <Td>{d.cure_mechanism}</Td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {isComplete && allocationLedger && (
        <section style={s.section}>
          <h2 style={s.sectionTitle}>Waterfall Allocation</h2>
                ...
                {allocationLedger.entries.map(e => {
                  ...
                  return (
                    <tr key={e.step_id} style={blocked ? { opacity: 0.55 } : diverted ? { background: 'var(--color-flag-tint)' } : {}}>
                      ...
                      <Td style={partial ? { color: 'var(--color-fail)', fontWeight: 600 } : {}}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
        <button style={s.btnBack} onClick={() => setScene(2)}>← Back to Coverage Tests</button>
        <button
          style={{ ...s.btnPrimary, opacity: isComplete ? 1 : 0.5 }}
          disabled={!isComplete}
          onClick={() => setScene(4)}
        >
          Generate Report →
...
function FormRow({ label, children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
      <label style={{ width: 200, fontSize: 13, color: 'var(--color-text-muted)', flexShrink: 0 }}>{label}</label>
      {children}
    </div>
  );
}
...
const s = {
  ...
  formGrid:       { maxWidth: 480 },
  input:          { padding: '6px 10px', border: '1px solid var(--color-border)', borderRadius: 5, fontSize: 13, width: 220 },
  ...
  diversionBanner:{ padding: '10px 16px', background: 'var(--color-flag-tint)', border: '1px solid var(--color-flag-border)', borderRadius: 6, marginBottom: 12, fontSize: 13, fontWeight: 600, color: '#7a5c00' },
